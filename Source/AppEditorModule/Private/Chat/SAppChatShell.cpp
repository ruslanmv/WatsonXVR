...
class SAppChatSessionOption : public STableRow<FNamePtr>
{
public:
	SLATE_BEGIN_ARGS(SAppChatSessionOption)
		{
		}
	...
	void Construct(const FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTableView, const FNamePtr& InItem)
	{
		...
		STableRow<FNamePtr>::Construct(
			...
			.Text(this, &SAppChatSessionOption::GetName)
			.ToolTipText(this, &SAppChatSessionOption::GetName)], InOwnerTableView);
	}
	...
	void EnableEditMode()
	{
		const TSharedRef<STextEntryPopup> TextEntry = SNew(STextEntryPopup).Label(FText::FromString("Rename Session")).OnTextCommitted(
				this, &SAppChatSessionOption::OnNameCommited);
...
void SAppChatShell::Construct([[maybe_unused]] const FArguments&)
{
	ChildSlot
	[
		ConstructContent()
...
SAppChatShell::~SAppChatShell() = default;

TSharedRef<SWidget> SAppChatShell::ConstructContent()
{
	return SNew(SHorizontalBox)
		...
		[
			...
			.OnGenerateRow(this, &SAppChatShell::OnGenerateChatSessionRow)
			.OnSelectionChanged(this, &SAppChatShell::OnChatSessionSelectionChanged)
			.SelectionMode(ESelectionMode::Single)
			.ClearSelectionOnClick(false)
			.OnMouseButtonDoubleClick(this, &SAppChatShell::OnChatSessionDoubleClicked)
			.OnKeyDownHandler(this, &SAppChatShell::OnChatSessionKeyDown)
...
void SAppChatShell::InitializeChatSessionOptions()
{
	ChatSessions.Empty();

	if (const FString SessionsPath = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("App")); FPaths::DirectoryExists(SessionsPath))
	{
...
void SAppChatShell::InitializeChatSession(const FNamePtr& InItem)
{
	if (!ShellBox.IsValid())
	{
		return;
	...
	ShellBox->SetContent(SAssignNew(CurrentView, SAppChatView).SessionID(*InItem));
...
TSharedRef<ITableRow> SAppChatShell::OnGenerateChatSessionRow(FNamePtr InItem, const TSharedRef<STableViewBase>& OwnerTable)
{
	return SNew(SAppChatSessionOption, OwnerTable, InItem).OnNameChanged(this, &SAppChatShell::OnChatSessionNameChanged);
}

void SAppChatShell::OnChatSessionSelectionChanged(const FNamePtr InItem, [[maybe_unused]] ESelectInfo::Type SelectInfo)
{
	if (!InItem.IsValid())
	{
		return;
...
void SAppChatShell::OnChatSessionNameChanged(const FNamePtr InItem, const FName& NewName)
{
	if (!InItem.IsValid())
	{
		return;
	...
	if (const FString SessionPath = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("App"), InItem->ToString()); FPaths::FileExists(SessionPath))
	{
		const FString NewPath = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("App"), NewName.ToString());
...
void SAppChatShell::OnChatSessionDoubleClicked(const FNamePtr InItem)
{
	...
	{
		if (const TSharedPtr<SAppChatSessionOption> Session = StaticCastSharedPtr<SAppChatSessionOption>(Row); Session.IsValid())
		{
...
FReply SAppChatShell::OnChatSessionKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	if (!ChatSessionListView.IsValid())
	{
	...
	if (const FString SessionPath = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("App"), SelectedItem->ToString() + TEXT(".json"));
