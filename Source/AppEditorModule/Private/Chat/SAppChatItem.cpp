...
void SAppChatItem::Construct(const FArguments& InArgs)
{
	MessageRole = InArgs._MessageRole;
	InputText = InArgs._InputText;

	if (MessageRole == EAppChatRole::Assistant)
	{
		MessagingHandlerObject = NewObject<UAppMessagingHandler>();
...
TSharedRef<SWidget> SAppChatItem::ConstructContent()
{
	...
	else if (MessageRole == EAppChatRole::System)
	{
...
FString SAppChatItem::GetRoleText() const
{
	return Role.IsValid() ? Role->GetText().ToString() : FString();
}

FString SAppChatItem::GetMessageText() const
{
